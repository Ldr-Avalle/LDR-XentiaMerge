report 50081 "compras sin facturar"
{
    ...
    dataset
    {
        ...
        {
            DataItemTableView = sorting("Document Type", "Document No.", "Line No.") where("Document Type" = filter(Order | "Return Order"), Type = filter(Item), "Quantity Invoiced" = const(0), "Buy-from Vendor No." = filter(<> 'P00040'));
